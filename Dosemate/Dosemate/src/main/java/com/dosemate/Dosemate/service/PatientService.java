...
public class PatientService {
   ...
   public PatientInfo saveNewPatient(PatientInfo patient, int doctorId) {
    ...
    if (!doctor.isApproved()) {
        throw new RuntimeException("Doctor is not approved by admin.");
    }
    ...
    PatientInfo finalPatient = patientRepo.findByPhoneNo(phoneNo)
            ...
            .map(existing -> {
                ...
                if (patient.getDoctorExaminations() != null) {
                    existing.getDoctorExaminations()
                            .addAll(patient.getDoctorExaminations());
                }

                if (patient.getMedicines() != null) {
                    existing.getMedicines()
                            .addAll(patient.getMedicines());
                }

                if (patient.getOtherTreatments() != null) {
                    existing.getOtherTreatments()
                            .addAll(patient.getOtherTreatments());
                }
    ...
    public PatientInfo addVisitToExistingPatient(PatientVisitDTO dto, int doctorId) {
        ...
        if (dto.getDoctorExaminations() != null) {
            patient.getDoctorExaminations()
                    .addAll(dto.getDoctorExaminations());
        }
        ...
        // ❌ HISTORY STORAGE REMOVED COMPLETELY
    ...
    // ------------------- SMS -------------------
    private String buildMedicineSms(
            PatientInfo patient,
            Doctor doctor,
            List<Medicine> medicines,
            List<OtherTreatment> treatments,
        ...
        if (isNewPatient) {
            sb.append("Hello ").append(patient.getName()).append(", this is ")
                    .append(doctor.getDoctorName()).append(".\n");
            sb.append("You visited our hospital.\n");
        } else {
            sb.append("Hello ").append(patient.getName()).append(", this is Dr. ")
                    .append(doctor.getDoctorName()).append(".\n");
            sb.append("Your updated medicines:\n");
        }

        sb.append("\nName | Session | Method | Qty | Total\n");
